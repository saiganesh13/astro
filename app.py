...
def get_lahiri_ayanamsa(year):
    ...
    Based on N.C. Lahiri's formula with polynomial terms
    """
    # Julian centuries from J2000.0
    t = (year - 2000.0) / 100.0
    
    # Lahiri ayanamsa formula (more accurate with higher order terms)
    # Base value at 2000.0 is approximately 23.85°
    ayanamsa = (23.85 + 
                50.2388 * t +           # Linear term (arc-seconds per century)
                0.000163 * t * t -       # Quadratic term
                0.0000052 * t * t * t)   # Cubic term
    
    # Convert from arc-seconds to degrees for linear term
    ayanamsa = 23.85 + (50.2388/3600.0) * (year - 2000) + 0.000163 * t * t - 0.0000052 * t * t * t
...
def filter_from_birth(periods, birth_dt):
    """Filter periods from birth date"""
    out = []
...
def compute_chart(name, date_obj, time_str, lat, lon, tz_offset, max_depth):
    ...
    t = Time(utc_dt)
    jd = t.jd
    ayan = get_lahiri_ayanamsa(utc_dt.year + (utc_dt.month-1)/12.0 + (utc_dt.day-1)/365.25)
    ...
    for p in ['sun','moon','mars','mercury','jupiter','venus','saturn','rahu','ketu']:
        ...
        planet_data[planet_cap] = {
            'sthana': sthana, 'volume': volume, 'good_volume': good_volume, 
            'bad_volume': bad_volume, 'dig_bala': dig_bala, 'L': L, 
    ...
    for h in range(1, 13):
        ...
        if len(house_planets) > 1:
            ...
            general_grabbers.sort(key=lambda p: -order_dict.get(p, 0))
    ...
    for p in ['Sun','Moon','Mars','Mercury','Jupiter','Venus','Saturn','Rahu','Ketu']:
        ...
        note_str = '; '.join(notes)
        rows.append([p, f"{data['L']:.2f}", data['sign'], data['nak'], data['pada'], data['ld_sl'], 
                    f"{data['dig_bala']}%" if data['dig_bala'] is not None else '', 
                    f"{data['sthana']}%", 
                    f"{data['volume']:.2f}" if isinstance(data['volume'], float) else '', 
                    f"{data['good_volume']:.2f}" if isinstance(data['good_volume'], float) else '', 
                    f"{data['bad_volume']:.2f}" if isinstance(data['bad_volume'], float) else '', 
                    note_str])
    
    df_planets = pd.DataFrame(rows, columns=['Planet','Deg','Sign','Nakshatra','Pada','Ld/SL',
                                            'Dig Bala (%)','Sthana Bala (%)','Volume',
                                            'Good Volume','Bad Volume','Consumed Notes'])
    ...
    aspects_dict = {'Sun':[7],'Moon':[7],'Mars':[4,7,8],'Mercury':[7],
...
with c1:
    # CHANGED: Extended date range from year 1 to 2200
    birth_date = st.date_input(
        "Birth Date", 
        value=datetime.now().date(),
        min_value=datetime(1, 1, 1).date(), 
        max_value=datetime(2200, 12, 31).date(),
        help="Select any date from 1 AD to 2200 AD"
    )
with c2:
...
if use_custom_coords:
    ...
    with clat: 
        lat = st.number_input("Birth Latitude", value=13.08, format="%.4f")
    with clon: 
        lon = st.number_input("Birth Longitude", value=80.27, format="%.4f")
else:
    ...
    if birth_city_query and len(birth_city_query) >= 2:
        try:
            ...
            st.session_state.search_results = [{'display': loc.address, 'lat': loc.latitude, 
                                               'lon': loc.longitude, 'address': loc.address} 
    ...
    if st.session_state.search_results:
        ...
        i = opts.index(sel)
        lat = st.session_state.search_results[i]['lat']
        lon = st.session_state.search_results[i]['lon']
...
if st.button("Generate Chart", use_container_width=True):
    if not name:
        ...
        try:
            with st.spinner("Calculating chart with improved accuracy..."):
                st.session_state.chart_data = compute_chart(name, birth_date, birth_time, lat, lon, tz_offset, max_depth)
            st.success("Chart generated successfully with improved planetary positions and dasha calculations!")
            st.rerun()
...
if st.session_state.chart_data:
    ...
    st.caption("Check which planetary period is currently active based on your location")
    ...
    with cc2:
        depth_choice = st.selectbox(
            ...
            index=0,
            help="Select the depth level for micro-period analysis"
        )
        ...
        if not current_city_query or len(current_city_query) < 2:
            ...
            try:
                ...
                if not cur_locs:
                    ...
                    st.success(f"📍 Detected time zone: {tz.zone} • Local now: {now_local.strftime('%Y-%m-%d %H:%M:%S')}")
                    
                    if active_path:
                        st.markdown("**🔸 Active Planetary Stack (Dasa → … → " + depth_choice + ")**")
                        rows = []
                    ...
                    if idx is not None:
                        ...
                        for i, (lord, s, e) in enumerate(nxt):
                        ...
                        if len(nxt) > 0:
                            current_lord, current_start, current_end = nxt[0]
                            time_remaining = current_end - now_utc_naive
                            days_remaining = time_remaining.total_seconds() / 86400
                            
                            if days_remaining < 1:
                                hours_remaining = days_remaining * 24
                                minutes_remaining = (hours_remaining - int(hours_remaining)) * 60
                                st.info(f"⏳ Time remaining in current {depth_choice}: {int(hours_remaining)}h {int(minutes_remaining)}m")
                            else:
                                years_remaining = int(days_remaining / TROPICAL_YEAR_DAYS)
                                rem_days = days_remaining % TROPICAL_YEAR_DAYS
                                months_remaining = int(rem_days / 30.4375)
                                days_only = int(rem_days % 30.4375)
                                st.info(f"⏳ Time remaining in current {depth_choice}: {years_remaining}y {months_remaining}m {days_only}d")
                    
                    st.caption("Note: Micro-period boundaries are calculated in UTC and displayed in your city's local time zone. Calculations use accurate tropical year (365.242189 days).")
